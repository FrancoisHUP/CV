...
const Fairy = ({ onChatOpen, isChatOpen, aiMessage }: FairyProps) => {
  ...
  const [lastMessageChange, setLastMessageChange] = useState<number>(0);
  const bobbingStartTimeRef = useRef<number | null>(null);
  const originalScale = new THREE.Vector3(1, 1, 1);

  // Update lastMessageChange whenever aiMessage changes.
  useEffect(() => {
    if (aiMessage) {
      setLastMessageChange(performance.now() / 1000);
    }
  }, [aiMessage]);
      ...
      gsap.to(ref.current.position, {
        ...
        onComplete: () => {
          if (ref.current) {
            ...
            setBillboardFixedPosition(finalPos); // Fix the billboard position
          }
  ...
  useEffect(() => {
    if (!isChatOpen && ref.current && !wandering) {
      ...
      gsap.to(ref.current.position, {
        ...
        onComplete: () => {
          setWandering(true);
          setBillboardFixedPosition(null); // Remove fixed position when chat closes
        },
      });
      // Reset sphere scale when leaving chat mode.
      if (ref.current) {
        ref.current.scale.copy(originalScale);
      }
    }
  }, [isChatOpen, originalPosition, wandering]);
  ...
  useFrame(() => {
    ...
    if (isChatOpen && chatTarget && ref.current && transitionComplete) {
      ...
      // Apply speaking pulse if text is actively updating
      if (aiMessage && currentTime - lastMessageChange < PULSE_STOP_THRESHOLD) {
  ...
  return (
    <>
      ...
      {isChatOpen && aiMessage && billboardFixedPosition && (
        <Billboard position={billboardFixedPosition}>
          ...
          >
            ...
            >
              <div style={{ fontFamily: "Arial" }}>{aiMessage}</div>
            </div>
          </Html>
        </Billboard>
      )}
